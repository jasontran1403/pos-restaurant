...
const SwipeToConfirmSwap = (props) => {
  ...
  const handleSwipe = () => {
    ...
    const data = JSON.stringify({
      ...
      type: props.type,
      toWalletAddress: props.toWalletAddress,
      amount: props.amount,
    });
    ...
    Axios.request(config)
      .then((response) => {
        if (response.data === "ok") {
          toast.success(`Swap ${props.amount} successful.`, {
            position: "top-right",
            autoClose: 1500,
            onClose: () => {
  ...
  return (
    <button
      ...
      className={`btn ${loading || actionCompleted ? "action" : ""}`}
      onClick={handleSwipe}
    >
      {loading ? (
...
export default SwipeToConfirmSwap;
